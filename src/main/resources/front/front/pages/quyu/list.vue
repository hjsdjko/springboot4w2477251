		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"20rpx 24rpx","background":"#fff","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #A1A1A1","padding":"12rpx 20rpx 12rpx 80rpx","color":"#000","borderRadius":"10rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.quyu" type="text" placeholder="区域" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"24rpx","margin":"0","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","padding":"20rpx 0 20rpx 20rpx","margin":"0 0 20rpx","overflow":"hidden","borderRadius":"10rpx","flexWrap":"wrap","flexDirection":"column","display":"flex","width":"100%","height":"340rpx"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"calc(100% - 260rpx)","padding":"20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('quyu','修改')) || (!userid && isAuthFront('quyu','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('quyu','删除')) || (!userid && isAuthFront('quyu','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"calc(100% - 260rpx)","padding":"20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('quyu','修改')) || (!userid && isAuthFront('quyu','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"border":"2rpx solid #000","padding":"4rpx 20rpx","borderRadius":"40rpx","display":"flex"}' v-if="(userid && isAuth('quyu','删除')) || (!userid && isAuthFront('quyu','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#000","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`quyu`, params);
                } else {
                    res = await this.$api.list(`quyu`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('quyu', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.quyu){
					searchForm['quyu'] = '%' + this.searchForm.quyu + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`quyu`, searchForm);
                } else {
                    res = await this.$api.list(`quyu`, searchForm);
                }
				// 如果是第一页数据置空
