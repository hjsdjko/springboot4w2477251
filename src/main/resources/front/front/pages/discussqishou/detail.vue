	...
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
            <view :style='{"padding":"24rpx","boxShadow":"0 8rpx 20rpx rgba(0,0,0,.3)","margin":"20rpx auto","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 40rpx)","height":"auto"}' class="detail-content">
				...
				<view class="detail-list-item" :style='{"margin":"0 0 24rpx 0","borderColor":"#D8D8D8","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"auto","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#000","textAlign":"right"}'>评分：</view>
					<view  class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}'>{{detail.score}}</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx 24rpx","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx","color":"rgb(255, 255, 255)","borderRadius":"10rpx","background":"rgb(255, 170, 51)","width":"calc(100% / 3 - 20rpx)","fontSize":"26rpx","height":"80rpx"}' v-if="userid&&isAuth('discussqishou','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0 10rpx 10rpx","color":"rgb(255, 255, 255)","borderRadius":"10rpx","background":"rgb(255, 170, 51)","width":"calc(100% / 3 - 20rpx)","fontSize":"26rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('discussqishou','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussqishouCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussqishouCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussqishou', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussqishou');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussqishou');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussqishou', this.id);
				this.detail = res.data;

				if(type==2){
					this.detail.discussnum++
					await this.$api.update('discussqishou',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussqishou', {
						page: mescroll.num,
						limit: 10,
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussqishou/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussqishou',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussqishou',that.detail)
							setTimeout(()=>{
